...
openAiRoutes.post("/chat/completions", async (c) => {
  ...
  try {
    ...
    for (let attempt = 0; attempt < maxRetry; attempt++) {
      ...
      try {
        const uploads = await mapLimit(imgInputs, 5, (u) => uploadImage(u, cookie, settingsBundle.grok, c.env));
...
openAiRoutes.post("/images/edits", async (c) => {
  ...
  try {
    ...
    for (const file of files) {
      ...
      const uploaded = await uploadImage(dataUrl, cookie, settingsBundle.grok, c.env);
